...
/*
* https://rvls.io/
* https://discord.gg/vguJtdbgtu
* https://t.me/+QND4dl1fwgFhOWEx
* https://x.com/RVLStoken/
* https://instagram.com/rvlstoken/
* Developed by RVLS Team
...
contract RVLS is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"RVLS"; 
    string private constant _symbol = unicode"RVLS";
    ...
    constructor (address oldTokenAddress) {
        ...
        oldToken = IOldERC20(oldTokenAddress);
    }
    ...
    function migrateTokens(uint256 amount) external {
        require(amount > 0, "Migration amount must be greater than zero");
        uint256 oldTokenBalance = oldToken.balanceOf(_msgSender());
        require(oldTokenBalance >= amount, "Insufficient balance in old contract");

        // Transfer old tokens to this contract and burn them
        oldToken.transferFrom(_msgSender(), address(this), amount);
        oldToken.burn(amount);

        // Mint new tokens to the user in the new contract
        _balances[_msgSender()] = _balances[_msgSender()].add(amount);
        _tTotal = _tTotal.add(amount);
        emit Transfer(address(0), _msgSender(), amount);

        emit TokensMigrated(_msgSender(), amount);
    }
