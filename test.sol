/**
 *Submitted for verification at BscScan.com on 2024-04-25
*/
...
contract TEST is ERC20, Ownable, ReentrancyGuard {
    ...
    event ChangedWallets(address marketing, address development);

    constructor() ERC20("TEST", "TEST") Ownable(msg.sender) {
        uint256 initialSupply = 1000000 * 10**decimals();
        _mint(msg.sender, initialSupply);
        swapTokenAtAmount = (totalSupply() * 1) / feeDenominator; // 0.01% of total supply
    ...
    function _transfer(
        ...
        if (exemptFee[sender] || exemptFee[recipient]) {
            ...
            if (
                ...
                uint256 taxAmount = (amount * taxRates) / feeDenominator;
    ...
    function distributeTaxes(uint256 contractTokenBalance) internal {
        ...
        _swapTokensForBNB(remainingTokens);
        uint256 remainingBNB = address(this).balance;

        uint256 half = remainingBNB / 2;
        sendBNB(marketing, half);
        sendBNB(development, half);
    }

    function changeTaxWallets(address _marketing, address _development)
        external
        onlyOwner
    {
        ...
        emit ChangedWallets(_marketing, _development);
    }

    function sendBNB(address _to, uint256 amount)
        internal
        nonReentrant
        returns (bool)
    {
    ...
    function changeSwapTokenAtAmount(uint256 newAmount) external onlyOwner {
        require(
            newAmount > 0 && newAmount < (totalSupply() / 100),
