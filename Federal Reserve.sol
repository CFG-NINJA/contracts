...
contract FederalReserve is ERC20, Auth {
    ...
    event SetFeeReceivers(address _liquidityReceiver, address _marketingReceiver, address _lotteryFeeReceiver, address _opsFeeReceiver, address _bondReceiver);
    ...
    address private lotteryReceiver = 0xD2Ea513eB1D5D69fa5d9d99dEA4bBCAdF2ff8e2b;
    ...
    IStimulusCheck public stimulusCheck = IStimulusCheck(address(lotteryReceiver));
    IDEXRouter public router;
    address public pair;
    mapping(address => uint256) public stimulusCountdown;
    uint256 stimulusCooldown = 7 days;
    uint256 stimulusCheckThreshold = 1000;
    bool stimulusActivated = false;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (automatedMarketMakerPairs[sender]) { //buy
            if (!isFeeExempt[recipient]) {
                ...
                if (amount > getEquivalentValue().mul(100).div(stimulusCheckThreshold) && block.timestamp >= stimulusCountdown[recipient] && stimulusActivated) {
    ...
    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _lotteryFeeReceiver, address _opsFeeReceiver, address _bondReceiver) external authorized {
        require(_autoLiquidityReceiver != address(0) && _marketingFeeReceiver != address(0) && _lotteryFeeReceiver != address(0) && _opsFeeReceiver != address(0) && _bondReceiver != address(0), "Zero Address validation");
        autoLiquidityReceiver = _autoLiquidityReceiver;
        marketingFeeReceiver = _marketingFeeReceiver;
        lotteryReceiver = _lotteryFeeReceiver;
        opsFeeReceiver = _opsFeeReceiver;
        bondReceiver = _bondReceiver;

        emit SetFeeReceivers(_autoLiquidityReceiver, _marketingFeeReceiver, _lotteryFeeReceiver, _opsFeeReceiver, _bondReceiver);
    }
    ...
    function getEquivalentValue() internal view returns (uint256) {
        ...
        return _FederalReserve.div(BNB).div(10).mul(1e18);
    }
    ...
    function updateStimulusCheckThreshold(uint256 _number) external authorized {
        stimulusCheckThreshold = _number;
    }
}