...
contract RVLSToken is ERC20, Ownable, ReentrancyGuard {
    uint256 public constant TAX_PERCENT = 2; // Tax percent fixed at 2%
   ...
   constructor(address _donationsWallet, address _owner, address _oldToken) ERC20("Rivals Token", "RVLS") {
        ...
        lockReleaseTime = 1725235200; // August 1, 2024 (2 years after August 2024)
        ...
        _mint(address(this), lockAmount); // Mint and lock 15% of the total supply
        _mint(msg.sender, MAX_SUPPLY - lockAmount - p2eAmount - stakingAndLiquidityAmount - marketingAmount - charityAmount - communityRewardsAmount - presaleAmount); // Mint the remaining tokens to the contract creator
        _mint(address(this), p2eAmount); // Mint P2E incentives to the contract itself
        _mint(address(this), stakingAndLiquidityAmount); // Mint staking rewards and liquidity to the contract itself
        _mint(address(this), marketingAmount); // Mint marketing tokens to the contract itself
        _mint(address(this), charityAmount); // Mint charity tokens to the contract itself
        _mint(address(this), communityRewardsAmount); // Mint community rewards tokens to the contract itself
        _mint(msg.sender, initialPresaleRelease); // Mint initial presale release to the contract creator
        _mint(address(this), vestedPresaleAmount); // Mint vested presale tokens to the contract itself
    }
    ...
    function distributeP2EIncentives(address to, uint256 amount) external {
     ...
     function _transfer(address sender, address recipient, uint256 amount) internal override {
        require(amount <= (totalSupply() * TAX_PERCENT) / 100, "Transfer amount exceeds the max tax limit");
