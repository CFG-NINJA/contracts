...
contract TOM is Context, Ownable, IERC20, ERC20Detailed {
	...
	uint256 public _totalFee = liquidityFee + walletSFee + walletTFee + walletVFee + walletXFee + walletYFee + walletZFee + buyBackBurnFee + foundationFee;
    
    address payable public liquidityWallet = payable(0x5FD69eaB85d1a52981f2EF657B06Da8f70eD9E8a);
    ...
    uint256 public numTokensSellToAddToLiquidity = 750000 * 10**18;
 
    event walletChanged(address oldWallet, address newWallet);
    event MinTokensBeforeSwapUpdated(uint256 minTokensBeforeSwap);
    event SwapAndLiquifyEnabledUpdated(bool enabled);
    event tokenRemoved(address owner);
    event numTokensSellToAddToLiquidityChanged(uint value);
    ...
    function changeLiquidityWallet(address payable newWallet) external onlyOwner
    {
        require(newWallet != address(0), "Address Zero detected!!");
        liquidityWallet = newWallet;
        emit walletChanged(liquidityWallet, newWallet);
        _isExcludedFromFee[liquidityWallet] = true;
    }
    function changeSWallet(address payable newWallet) external onlyOwner
    {
        require(newWallet != address(0), "Address Zero detected!!");
        SWallet = newWallet;
        emit walletChanged(SWallet, newWallet);
        _isExcludedFromFee[SWallet] = true;
    }
    ...
    function changeTWallet(address payable newWallet) external onlyOwner
    {
        require(newWallet != address(0), "Address Zero detected!!");
        TWallet = newWallet;
        emit walletChanged(TWallet, newWallet);
        _isExcludedFromFee[TWallet] = true;
    }
    ...
    function changeVWallet(address payable newWallet) external onlyOwner
    {
        require(newWallet != address(0), "Address Zero detected!!");
        VWallet = newWallet;
        emit walletChanged(VWallet, newWallet);
        _isExcludedFromFee[VWallet] = true;
    }
    ...
    function changeXWallet(address payable newWallet) external onlyOwner
    {
        require(newWallet != address(0), "Address Zero detected!!");
        XWallet = newWallet;
        emit walletChanged(XWallet, newWallet);
        _isExcludedFromFee[XWallet] = true;
    }
    ...
    function changeYWallet(address payable newWallet) external onlyOwner
    {
        require(newWallet != address(0), "Address Zero detected!!");
        YWallet = newWallet;
        emit walletChanged(YWallet, newWallet);
        _isExcludedFromFee[YWallet] = true;
    }
    ...
    function changeZWallet(address payable newWallet) external onlyOwner
    {
        require(newWallet != address(0), "Address Zero detected!!");
        ZWallet = newWallet;
        emit walletChanged(ZWallet, newWallet);
        _isExcludedFromFee[ZWallet] = true;
    }
    ...
    function changeBuybackWallet(address payable newWallet) external onlyOwner
    {
        ...
        emit walletChanged(buyBackBurnWallet, newWallet);
        _isExcludedFromFee[buyBackBurnWallet] = true;
    }
    ...
    function changeFoundationWallet(address payable newWallet) external onlyOwner
    {
        ...
        emit walletChanged(foundationWallet, newWallet);
        _isExcludedFromFee[foundationWallet] = true;
    }
    ...
    function changePresaleWallet(address payable newWallet) external onlyOwner
    {
        ...
        emit walletChanged(presaleWallet, newWallet);
        _isExcludedFromFee[presaleWallet] = true;
    }
    ...
    uint256 _foundationFee) external onlyOwner {
        ...
        _totalFee = liquidityFee + walletSFee + walletTFee + walletVFee + walletXFee + walletYFee + walletZFee + buyBackBurnFee + foundationFee;
        require(_totalFee <= 500, "Fee is crossing the boundaries");
        emit FeeChanged(_totalFee);
    }
    ...
    {
        require(_numTokensSellToAddToLiquidity > 0, "Invalid entry");
    ...
    function _transfer(address sender, address recipient, uint amount) internal{
        ...
        if(takeFee)
        {
            uint256 taxAmount = (amount * (liquidityFee + walletSFee + walletTFee + walletVFee + walletXFee + walletYFee + walletZFee
    ...
    {
        return (liquidityFee + walletSFee + walletTFee + walletVFee + walletXFee + walletYFee + walletZFee
      ...
      function swapAndLiquify(uint256 tokensToLiquify) private {
        ...
        uint256 ethForLiquidity = ethBalance * (liquidityFee) / totalFee();
        ...
        payable(liquidityWallet).transfer(ethForLiquidity);
